...
public class HomeForm extends javax.swing.JPanel {
    ...
    public void setResponseAPDU(byte[] datares, short le,info info) {
        int status1 = theXP.resAPDU.getSW1();
        System.out.println("status1:" + status1);
        int status2 = theXP.resAPDU.getSW2();
        System.out.println("status2:" + status2);
    ...
    public void saveUserToDatabase(info info) {
           String tv = "INSERT INTO user VALUES ("+info.getCmnd()+","+info.getHoten()  ;
    }
    ...
    private void btn_sendataActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_sendataActionPerformed
        if (connected == true) {
            if (input == false) {
                ...
                String arraysend = cmnd.concat(".").concat(hoten)
                        .concat(".").concat(sdt)
                        .concat(".").concat(pin);
//                        .concat(".").concat(sdt)
//                        .concat(".").concat(cmnd);
                ...
                String tvSQL2 = "Select * from user where cmnd = " + cmnd + "";
                ResultSet rsdv = Main.connection.HienThongTin(tvSQL2);
                if (rsdv != null) {
